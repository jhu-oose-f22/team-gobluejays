...
using PrimaryKey = std::variant<std::monostate, int64_t, InternString, GlobalKey, ObjectId>;

struct Path {
    using Element = std::variant<InternString, uint32_t>;
    ...
    {
        return !m_path.empty() && std::holds_alternative<uint32_t>(m_path.back());
    }

    uint32_t& index() noexcept
    {
        REALM_ASSERT(is_array_index());
        return std::get<uint32_t>(m_path.back());
    }
...
struct AddTable : TableInstruction {
    ...
    std::variant<PrimaryKeySpec, EmbeddedTable> type;
...
#undef REALM_INSTRUCTION_VARIANT_ALTERNATIVE
    ...
    {
        return std::holds_alternative<Vector>(m_instr);
    }
...
inline decltype(auto) Instruction::visit(F&& lambda)
{
    ...
    if (std::holds_alternative<Vector>(m_instr)) {
        REALM_TERMINATE("visiting instruction vector");
    }
#define REALM_VISIT_VARIANT(X)                                                                                       \
    else if (std::holds_alternative<Instruction::X>(m_instr))                                                        \
    {                                                                                                                \
        return lambda(std::get<Instruction::X>(m_instr));                                                            \
    }
...
inline T* Instruction::get_if() noexcept
{
    ...
    if constexpr (std::is_same_v<TableInstruction, T>) {
        ...
        return std::get_if<T>(&m_instr);
    }
}

inline size_t Instruction::size() const noexcept
{
    if (auto vec = std::get_if<Vector>(&m_instr)) {
        return vec->size();
    }
    return 1;
}
...
{
    if (auto v = std::get_if<Vector>(&m_instr)) {
        return *v;
    }
    else {
        Vector vec;
